...
namespace AsteroidAttack
{
    static class DrawHelper
    {
        ...
        public static void DrawGrid(Vector3 CameraOffset, Vector2 gridSize, Color color, float thickness = 2f, float spaceWidth = 50)
        {
            ...
            float startx = -CameraOffset.X +CameraOffset.X % spaceWidth - spaceWidth;
            float endx = gridSize.X -CameraOffset.X;

            float starty =  - CameraOffset.Y + CameraOffset.Y % spaceWidth - spaceWidth;
            float endy = gridSize.Y - CameraOffset.Y;
